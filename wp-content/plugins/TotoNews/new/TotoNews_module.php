...
function createOneSpanTotoNews($headText, $buttonText, $rowNum, $colNum, $keywordsStFromButtons, $keywordsStFromButtonsForGoogle, $theLan, $categoryForThisButton="", $subRowNumSt, $useFloatDiv, $useSeanWayToTest, $idNum, $language, $theISO2, $textInRowLan){  
			...
			foreach($variables as $configs)
			{
				if(count($configs)>0)
				{
					...
					foreach($configs as $config)
					{
						...
						$theKeywords, $theKeywordsForGoogle, "", $config->category, $subRowNumSt, $useFloatDiv, $useSeanWayToTest, $idNum, $lanInUrl, $config->iso2, $textInRowLan); //for now, language=""
						$idNum=$idNum+1;
						$output .= ", ";
						$icount++;
					}
